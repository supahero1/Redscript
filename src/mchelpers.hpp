...
#define MC_VARIABLE_CREATE_DEF(scope, type, ...) MC_DATA(modify, RS_PROGRAM_VARIABLES) \
                                                 PADL(set) PAD(value) \
                                                 MC_VARIABLE_JSON_DEFAULT(scope, type) \
                                                 __VA_ARGS__
#define MC_VARIABLE_SET_CONST(id, v) PADR(modify) RS_PROGRAM_DATA SEP ARR_AT(RS_PROGRAM_VARIABLES, STR(id)) PAD(.value set value) INS_L(v)
...
#pragma region stack
#define MC_STACK_PUSH_CONST(x) MC_DATA(modify, RS_PROGRAM_STACK) PAD(append value) INS_L(x)
